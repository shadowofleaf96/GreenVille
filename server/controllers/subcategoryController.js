...
export const createSubcategory = async (req, res) => {
  ...
  if (typeof subcategory_name === "string") {
    try {
      ...
      console.error("Error parsing subcategory_name:", error);
    }
  }
...
export const getAllSubcategories = async (req, res) => {
  ...
  const subcategoriesData = await queryBuilder.populate("category_id").lean();
...
export const updateSubcategoryById = async (req, res) => {
  ...
  if (typeof subcategory_name === "string") {
    try {
      ...
      console.error("Error parsing subcategory_name in update:", e);
    }
  }
  ...
  if (subcategory_name) {
    if (!subcategory_name.en || !subcategory_name.fr || !subcategory_name.ar) {
      return res.status(400).json({
        error: "Subcategory name must include 'en', 'fr', and 'ar' fields",
      });
    }
